...
| `contact` | 好友或群组名片 | `type`：名片类型，`user` 或 `group`<br>`user_id`：用户 ID<br>`user_tid`：用户临时 ID<br>`user_name`：用户昵称／用户名，可选<br>`group_id`：群组 ID<br>`group_tid`：群组临时 ID<br>`group_name`：群组名称，可选 |
