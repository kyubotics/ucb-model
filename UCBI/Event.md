...
2. 所有允许以星号开头的特殊字段或值，都是随翻译层程序的不同而不同的，它们是否统一化取决于 UCBI 实现中的相应适配器，因此即使同一个星号开头的字段名，在翻译层（即 `context` 中的 `via`）不同的情况下，也可能具有完全不同的含义，需谨慎使用。
...
数组中至少有一个元素。每个元素称为一个「消息段」，每个消息段又是一个数组，它的元素按顺序如下：

| 序号 | 数据类型 | 说明 |
| --- | ------- | --- |
| 0 | string | 消息段格式，后面具体说明 |
| 1 | string | 消息段内容，无论什么格式，都有这一项，对于纯文本消息段，这里就是文字内容，对于其它消息段，这里是人类可读的表示，如 `[图片]`、`[语音]`、`[表情:微笑]` |
| 2 | object | 其它信息，对于非纯文本消息段，这里存放具体的细节信息，后面具体说明 |
...
| 其它格式 | 其它特殊格式，以星号（`*`）开头，如 `*face` | - |

对于上面有定义的格式，消息段的第 2 项的 JSON 对象也可以包含特殊字段，同样以星号开头。
...
| `discuss_markname` | string | 讨论组备注名 | 可选 |
| `discuss` | string | 讨论组显示名（当有备注名时为备注名，否则为讨论组真实名称） | 可选 |
| 额外字段 | - | 对于某些翻译层特有的信息，允许使用额外字段来记录，字段名应以星号（`*`）开头，如 `*operator_id` | 可选 |
...
| 其它通知 | 对于某些有其它类型的通知的翻译层，允许使用特殊值，应以星号（`*`）开头，如 `*set_group_admin` | - |
